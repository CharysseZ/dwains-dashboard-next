...
export class DwainsDevicesCard extends LitElement {
  ...
  private _maintenanceSubtitle(buckets: Map<string, MaintenanceBucket>): string {
    ...
    if (summary.lowBatteryCount) {
      parts.push(`${summary.lowBatteryCount} 个低电量设备`);
    }
    if (summary.unavailableDeviceCount) {
      parts.push(`${summary.unavailableDeviceCount} 个不可用设备`);
    }
  ...
  private _formatMaintenanceState(state: any, kind: MaintenanceItem['kind']): string {
    if (kind === 'unavailable') {
      return state.state === 'unknown' ? '未知' : '不可用';
    }
  ...
  private _typeName(key: string): string {
    if (key === MAINTENANCE_KEY) return '维护';
    if (key === ENERGY_KEY) return '能源';
    if (key === DEVICES_OVERVIEW_KEY) return '概览';
      ...
      <nav class=${classMap(classes)}>
        ...
        <div class="area-list">
          ...
          >
            ...
            <div class="area-info">
              <div class="area-name">${this._typeName(DEVICES_OVERVIEW_KEY)}</div>
                ...
                >
                  ...
                  <div class="area-info">
                    ...
                    <div class="device-menu-subtitle">
                      ${newDevices.length === 1 ? '1 个新设备' : `${newDevices.length} 个新设备`}
                    </div>
                  </div>
                  ...
                  <div class="area-info">
                    ...
                    <div class="device-menu-subtitle">
                      ${energySummary.sensorCount === 1
                        ? '1 个实时功率传感器'
                        : `${energySummary.sensorCount} 个实时功率传感器`}
                    </div>
                  </div>
  ...
  private _renderDevicesOverview(
    ...
    if (showNewDevicesMenu) {
      cards.push({
        ...
        icon: 'mdi:new-box',
        title: '新设备',
        subtitle: newDevices.length === 1 ? '1 个新设备' : `${newDevices.length} 个新设备`,
    ...
    if (showEnergyMenu) {
      cards.push({
        ...
        icon: 'mdi:flash',
        title: '能源',
        subtitle: energySummary.sensorCount === 1 ? '1 个实时功率传感器' : `${energySummary.sensorCount} 个实时功率传感器`,
        ...
        ${this._renderDevicePageHeader({
          ...
          actions: html`
            <div class="energy-header-total">
              ...
              <small>${summary.sensorCount === 1 ? '1 个实时功率传感器' : `${summary.sensorCount} 个实时功率传感器`}</small>
            </div>
          `,
        })}
              ...
              <div class="energy-overview-grid">
                <section class="energy-overview-card total">
                  <div class="energy-overview-head">
                    ...
                    <div>
                      <h2>全屋</h2>
                      <p>${summary.sensorCount === 1 ? '1 个实时功率传感器' : `${summary.sensorCount} 个实时功率传感器`}</p>
                    </div>
                    <strong>${summary.formattedTotal}</strong>
                  </div>
                  ${this._renderEnergyStatisticsGraph(wholeHouseStatisticsEntities, '全屋用电历史')}
                </section>
    ...
    return html`
      <div class="device-view">
        ${this._renderDevicePageHeader({
          icon: 'mdi:new-box',
          title: '新设备',
          subtitle: `最近 ${NEW_DEVICE_WINDOW_HOURS} 小时内添加到 Home Assistant 的设备。`,
            ...
            ${devices.length
              ...
              : html`
                  <div class="recent-empty">
                    最近 ${NEW_DEVICE_WINDOW_HOURS} 小时内没有设备添加到 Home Assistant。
                  </div>
                `}
          </div>
        </section>
  ...
  private _formatAddedAge(createdAtMs: number): string {
    ...
    return `${Math.floor(hours / 24)}天前添加`;
  }
