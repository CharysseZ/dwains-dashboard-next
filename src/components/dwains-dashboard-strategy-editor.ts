...
export class DwainsDashboardStrategyEditor extends LitElement {
    ...
    return [
      {
        ...
        title: "仪表板",
        description: "名称和侧边栏图标。",
        summary: this._dashboardTitle || "当前仪表板",
      },
      {
        page: "home",
        group: "general",
        ...
        color: "#0ea5e9",
        title: "首页",
        description: "区域顺序、房屋信息和收藏。",
        summary: `${visibleHomeSections} 个区域 · ${visibleHouseInfoCards}/${DEFAULT_HOME_INFORMATION_CARDS.length} 个房屋卡片 · ${favoriteCount} 个收藏`,
      },
      {
        page: "header",
        group: "general",
        ...
        color: "#22a06b",
        title: "顶部栏 & 状态",
        description: "时间、天气、通知和警报芯片。",
        summary: `${this._config?.settings?.show_notifications === false ? "通知已隐藏" : "通知已显示"} · ${this._config?.settings?.alarm_entity_id ? "已选择警报" : "未选择警报"}`,
      },
      {
        page: "people",
        group: "layout",
        ...
        color: "#8b5cf6",
        title: "人员",
        description: "选择在 Dwains Dashboard 中可见的人员。",
        summary: `${personCount} 人`,
      },
      {
        page: "areas",
        group: "layout",
        ...
        color: "#14b8a6",
        title: "区域",
        description: "可见房间、房间顺序和不可用实体行为。",
        summary: `${areaCount} 个区域 · ${areasUnavailableMode}`,
      },
      {
        page: "devices",
        group: "layout",
        ...
        color: "#0891b2",
        title: "设备页面",
        description: "实体可见性和设备类型组。",
        summary: `${deviceTypeCount - hiddenDeviceTypeCount}/${deviceTypeCount} 个类型可见 · ${hiddenDeviceCount} 个隐藏设备 · ${devicesUnavailableMode}`,
      },
      {
        page: "replacements",
        group: "layout",
        ...
        color: "#7c3aed",
        title: "蓝图替换",
        description: "用蓝图卡片替换默认卡片。",
        summary: `${replacementCount} 个激活`,
      },
      {
        page: "permissions",
        group: "advanced",
      ...
      },
      {
        ...
        color: "#f59e0b",
        title: "支持",
        description: "捐赠链接和 SmartHomeShop.io。",
        summary: "可选",
      },
    ];
  }
  ...
  private _renderNotificationSettingsPanel() {
    return this._renderSettingsPanel(
      "mdi:bell-outline",
      "通知",
      "显示或隐藏 Dwains Dashboard 通知按钮和徽章。",
      html`
      ...
      <div class="home-layout-section">
        <div class="home-section-list ${this._draggedHomeSection ? 'dragging' : ''}">
          ${repeat(
            ...
            section => section,
            (section, index) => {
              const meta = getHomeSectionMeta(this.hass, section);
                ...
                >
                  ...
                  <div class="home-section-actions">
                    <button
                      ...
                      type="button"
                      title=${enabled ? this._t('settings.home_layout.hide_section') : this._t('settings.home_layout.show_section')}
                      aria-label=${enabled ? `${this._t('settings.home_layout.hide_section')} ${meta.label}` : `${this._t('settings.home_layout.show_section')} ${meta.label}`}
    ...
    return html`
      <div class="home-info-card-section">
        <div class="home-info-card-header">
          ...
          <span>${this._t('settings.house_info.visible', { visible: visibleCount, total: DEFAULT_HOME_INFORMATION_CARDS.length })}</span>
        </div>
        <div class="home-info-card-list">
          ${DEFAULT_HOME_INFORMATION_CARDS.map(card => {
            const meta = getHomeInformationCardMeta(this.hass, card);
    ...
    return html`
      <div class="device-types-visibility">
        <div class="device-types-header">
          ...
          <span>${visibleCount}/${options.length} 可见</span>
        </div>
    ...
    return html`
      <div class="device-admission-section">
        <div class="device-types-header">
          ...
          <span>${hiddenKnownDeviceCount}/${allDeviceIds.length} 隐藏</span>
        </div>
              ...
              return html`
                <ha-expansion-panel outlined ?expanded=${index === 0}>
                  <div slot="header" class="device-admission-panel-header" style=${`--device-type-color: ${group.color};`}>
                    ...
                    <small>${groupDeviceIds.length - hiddenInGroup}/${groupDeviceIds.length} 可见</small>
                  </div>
                          ...
                          <section class="device-admission-area">
                            <div class="device-admission-area-header">
                              <div>
                                ...
                                <span>${areaDeviceIds.length - hiddenInArea}/${areaDeviceIds.length} 可见</span>
                              </div>
      ...
      >
        ...
        <div class="device-admission-copy">
          ...
          <div class="device-type-count">
            ${device.entityCount === 1 ? this._t('entity.count.one') : this._t('entity.count', { count: device.entityCount })} · ${visible ? this._t('settings.device_visibility.visible_in_dd') : this._t('settings.device_visibility.hidden_in_dd')}
          </div>
        </div>
        <ha-switch
          .checked=${visible}
