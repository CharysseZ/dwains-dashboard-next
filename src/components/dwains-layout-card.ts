...
export class DwainsLayoutCard extends LitElement {
  ...
  private _syncBottomNavAreaContext(): void {
    ...
    window.dispatchEvent(new CustomEvent('dwains-dashboard-next-area-context-changed', {
      detail: {
        ...
        name: settingsSelected ? '设置' : area?.name || '首页',
  ...
  private _renderSidebarResizeHandle() {
    ...
    return html`
      <button
        ...
        type="button"
        title=${collapsed ? '显示区域侧边栏' : '折叠区域侧边栏'}
        aria-label=${collapsed ? '显示区域侧边栏' : '折叠区域侧边栏'}
  ...
  private _renderAreaButtons() {
    ...
    return sortedFloors.map(([floorName, areas]) => {
      const floorTitle = floorName === 'no_floor' ?
        this.hass.localize('ui.components.area-picker.no_floor') || '未分配区域' :
        floorName;
  ...
  private _statusCardActiveLabel(domain: DomainCount): { singular: string; plural: string } | undefined {
    ...
    if (domain.domain === 'light') return { singular: '灯光开启', plural: '灯光开启' };
    if (domain.domain === 'switch') return { singular: '开关开启', plural: '开关开启' };
    if (domain.domain === 'cover') return { singular: '窗帘打开', plural: '窗帘打开' };
    if (domain.domain === 'fan') return { singular: '风扇开启', plural: '风扇开启' };
    if (domain.domain === 'lock') return { singular: '锁解锁', plural: '锁解锁' };
    if (domain.domain === 'climate') return { singular: '空调运行', plural: '空调运行' };
    if (domain.domain === 'media_player') return { singular: '媒体播放中', plural: '媒体播放中' };
    if (domain.domain === 'vacuum') return { singular: '扫地机器人清扫中', plural: '扫地机器人清扫中' };
    if (domain.domain === 'alarm_control_panel') return { singular: '警报已布防', plural: '警报已布防' };

    if (domain.domain === 'binary_sensor') {
      switch (domain.deviceClass) {
        case 'door':
          return { singular: '门打开', plural: '门打开' };
        case 'window':
          return { singular: '窗户打开', plural: '窗户打开' };
        case 'opening':
          return { singular: '打开', plural: '打开' };
        case 'motion':
          return { singular: '检测到运动', plural: '检测到运动' };
        case 'smoke':
          return { singular: '检测到烟雾', plural: '检测到烟雾' };
        case 'gas':
          return { singular: '检测到燃气', plural: '检测到燃气' };
        case 'moisture':
          return { singular: '检测到潮湿', plural: '检测到潮湿' };
        case 'occupancy':
          return { singular: '检测到占用', plural: '检测到占用' };
        case 'presence':
          return { singular: '检测到存在', plural: '检测到存在' };
        case 'tamper':
          return { singular: '检测到 tampering', plural: '检测到 tampering' };
        case 'vibration':
          return { singular: '检测到振动', plural: '检测到振动' };
        case 'safety':
          return { singular: '安全激活', plural: '安全激活' };
        default:
          return { singular: `${domain.name} 激活`, plural: `${domain.name} 激活` };
      }
    }
  ...
  private _getHomeSummaryCards(): HomeSummaryCard[] {
    ...
    if (this._repairsIssueCount > 0) {
      cards.push({
        key: 'repairs',
        label: '维修',
        subtitle: `${this._repairsIssueCount} 个问题`,
        icon: 'mdi:wrench',
        color: '#f59e0b',
    ...
    if (this._discoveredDeviceCount > 0) {
      cards.push({
        key: 'discovered',
        label: '发现的设备',
        subtitle: `${this._discoveredDeviceCount} ${this._discoveredDeviceCount === 1 ? '个设备' : '个设备'} 待添加`,
        icon: 'mdi:devices',
        color: '#1494aa',
            ...
            <div class="welcome-user">
              ...
              <div class="welcome-copy">
                <div class="welcome-text">
                  ...
                  <span class="welcome-title">你好, ${userName}</span>
                </div>
        ...
        <div class="mobile-section-heading">
          <div class="mobile-section-title">
            <button
              ...
              type="button"
              title=${gridMode ? '滑动房屋信息' : '显示所有房屋信息'}
              aria-label=${gridMode ? '切换房屋信息到滑动卡片' : '显示所有房屋信息'}
        ...
        <div class="mobile-section-heading">
          <div class="mobile-section-title">
            <button
              class="mobile-layout-toggle ${gridMode ? 'active' : ''}"
              type="button"
              title=${gridMode ? '滑动摄像头' : '显示所有摄像头'}
              aria-label=${gridMode ? '切换摄像头到滑动卡片' : '显示所有摄像头'}
  ...
  private _renderHousePowerStatusCard() {
    ...
    const subtitle = powerUsage.sensorCount
      ? `${powerUsage.sensorCount} 个实时功率传感器`
      : '无实时功率传感器';
    ...
    return html`
      <div
        ...
        role="button"
        tabindex="0"
        aria-label=${`房屋用电: ${powerUsage.formattedTotal}`}
      >
        ...
        <div class="house-climate-head">
          ...
          <div class="house-climate-copy">
            ...
            <div class="house-climate-subtitle">
              ${climate.sensorCount === 1 ? '1 个气候传感器' : `${climate.sensorCount} 个气候传感器`}
            </div>
          </div>
        </div>
  ...
  private _formatPersonState(person: any): string {
    ...
    if (!person.state || person.state === 'unknown') return '未知';
    if (person.state === 'unavailable') return '不可用';
  ...
  private _getHomeSnapshotText(weatherEntity?: any): string {
    ...
    return parts.slice(0, 3).join(' · ') || '一切正常';
  }
  ...
  private _houseClimateMetric(
    ...
    return {
      kind,
      label: kind === 'temperature' ? '平均温度' : '平均湿度',
      value,
      count: values.length,
          ...
          <div class="mobile-section-title">
            ...
            ` : html`
              <button
                class="mobile-layout-toggle ${gridMode ? 'active' : ''}"
                type="button"
                title=${gridMode ? '滑动区域' : '显示所有区域'}
                aria-label=${gridMode ? '切换区域到滑动卡片' : '显示所有区域'}
        ...
        <div class="mobile-section-heading">
          <div class="mobile-section-title">
            <button
              class="mobile-layout-toggle ${gridMode ? 'active' : ''}"
              type="button"
              title=${gridMode ? '滑动收藏' : '显示所有收藏'}
              aria-label=${gridMode ? '切换收藏到滑动卡片' : '显示所有收藏'}
  ...
  private _favoriteQuickTitle(state: any, domain: string): string {
    const value = String(state?.state || '').toLowerCase();
    if (domain === 'cover') return ['open', 'opening'].includes(value) ? '关闭' : '打开';
    if (domain === 'lock') return value === 'unlocked' ? '解锁' : '上锁';
    if (['light', 'switch', 'fan', 'input_boolean'].includes(domain)) return value === 'off' ? '打开' : '关闭';
    return '详情';
  }
              ...
              <div class="mobile-domain-header">
                <div class="mobile-domain-title">
                  <button
                    class="mobile-layout-toggle ${gridMode ? 'active' : ''}"
                    type="button"
                    title=${gridMode ? '滑动卡片' : '显示所有卡片'}
                    aria-label=${gridMode ? '切换到滑动卡片' : '显示所有卡片'}
  ...
  private _renderUnavailableEntitiesIcon(areaId: string) {
    ...
    return html`
      <button
        ...
        title="显示 ${totalUnavailable} 个隐藏的不可用/未知实体"
      >
  ...
  private async _saveSettingsPage(): Promise<void> {
    ...
    try {
      ...
      this._settingsSaveError = `无法保存仪表板设置: ${String(err)}`;
    } finally {
      this._settingsSavePending = false;
    }
  }
  ...
  private async _loadPersistentNotifications(showError = true): Promise<void> {
    ...
    try {
      ...
      if (showError || this._notificationsOpen) {
        ...
        this._notificationsError = '无法加载 Home Assistant 持久通知。';
      }
    } finally {
  ...
  private _dismissPersistentNotification = async (notificationId: string) => {
    ...
    try {
      ...
      this._notificationsError = '无法忽略此通知。';
    }
  };
  ...
  private _dismissAllPersistentNotifications = async () => {
    ...
    try {
      ...
      this._persistentNotifications = previous;
      this._notificationsError = '无法忽略所有通知。';
    }
  };
